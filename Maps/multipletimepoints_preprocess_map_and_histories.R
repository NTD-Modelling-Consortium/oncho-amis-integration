...
mda_file = read_excel('Full_histories_df_popinfo_ALL_minimal_070425_listlabels.xlsx')
...
load('ALL_prevalence_map.rds') # load baseline map that has all IUs (endgame and non-endgame)
...
# add 2018 map
map_2018 = read.csv('maps_joint/samples_2018.csv',header=T) %>%
  mutate(IU_ID = paste0(substr(IU_code,1,3),substr(IU_code,(nchar(IU_code)-4),nchar(IU_code)))) %>%
  select(-X) %>%
  select(IU_ID,starts_with("X"))
...
for (id in 1:max(as.integer(iu_task_lookup$TaskID))){
  ...
  iu_file<-file.path("../EPIONCHO-IBM/model_output",paste0("IUs_MTP_",id,".csv"))
  ...
  for (iu in ius[1]) { # histories are all the same within a batch so only need to generate once
    ...
    mda_path<-file.path("../EPIONCHO-IBM/model_output",paste0("InputMDA_MTP_",id,".csv"))
    ...
    vc_all_years %<>%
      mutate(abr_multiplier = case_when(vector_control==1 ~ 0.2,
                                        vector_control==2 ~ 0,
                                        TRUE ~ 1))
    vc_path<-file.path("../EPIONCHO-IBM/model_output",paste0("InputVC_MTP_",id,".csv"))
